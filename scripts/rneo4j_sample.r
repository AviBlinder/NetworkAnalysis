...
#Part 1 - Connecting and creating Nodes + Relationships graph = startGraph("http://localhost:7474/db/data",username="neo4j",password="neo")
#clear(graph)

cypher(graph, "MATCH (p:Person) DETACH DELETE p") cypher(graph, "MATCH (c:Company) DETACH DELETE c")

nicole = createNode(graph, "Person", name="Nicole") greta = createNode(graph, "Person", name="Greta") kenny = createNode(graph, "Person", name="Kenny") adam = createNode(graph, "Person", name="Adam")
